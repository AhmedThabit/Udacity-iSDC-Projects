...
vector< vector<float> > initialize_beliefs(vector< vector<char> > grid) {
	...
	float Pfactor = 1.f / (rows*cols);
	vector< vector<float> >newGrid(rows, (vector<float>(cols, Pfactor)));
	...
	float p_miss) 
{
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		for (int j = 0; j < cols; ++j)
		{
			if (color==grid[i][j])
			{
				float newVal = (p_hit)*beliefs[i][j];
				newRow.push_back(newVal);
			} 
			else
			{
				float newVal = (p_miss)*beliefs[i][j];
				newRow.push_back(newVal);
			}
	...
	float blurring) 
{
	...
	vector<vector<float> > newGrid(rows, vector<float>(cols, 0.f));
