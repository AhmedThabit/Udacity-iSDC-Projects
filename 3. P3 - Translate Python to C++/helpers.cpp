...
//Inserting a function to create a matrix of zeroes
...
/*floatGrid zeros(int height, int width) {
	floatGrid newGrid(height, std::vector<float>(width,0.0));
	return newGrid;
}*/
...
*/
floatGrid normalize(floatGrid grid) {
...
/**
    ...
    @return - a new normalized two dimensional grid where probability 
    	   has been blurred.
*/
floatGrid blur(floatGrid grid, float blurring) {
	const int rows = grid.size(),
		cols = grid[0].size();

	const float centerProb = 1.0 - blurring,
		cornerProb = blurring / 12.0,
		adjacentProb = blurring / 6.0;
	...
	floatGrid newGrid = zeros(rows, cols);
			...
			{for (int dy = -1; dy <= 1; ++dy)
				{
					...
					int newI = (i + dy + rows) % rows;
					int newJ = (j + dx + cols) % cols;
